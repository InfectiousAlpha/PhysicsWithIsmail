...
export default function CourseM1Sim8({ simId, onScoreUpdate, onComplete }) {
  ...
  // Generate 10 mixed operations
  useEffect(() => {
    const generated = [];
    for (let i = 0; i < 10; i++) {
      let a = Math.floor(Math.random() * 19) - 9; // -9 to 9
      let b = Math.floor(Math.random() * 19) - 9; // -9 to 9
      let op = Math.random() > 0.5 ? '+' : '-';

      // Prevent overly simple 0 questions to keep it interesting
      if (a === 0) a = 2;
      if (b === 0) b = 3;

      generated.push({ a, b, op });
    }
  ...
  const handleSubmit = () => {
    ...
    let correctAns = 0;
    if (currentQ.op === '+') {
      correctAns = currentQ.a + currentQ.b;
    } else {
      correctAns = currentQ.a - currentQ.b;
    }

    const isCorrect = parseInt(userAnswer) === correctAns;
  ...
  const renderQuestion = () => {
    ...
    const bStr = q.b < 0 ? `(${q.b})` : `${q.b}`;
    return `${q.a} ${q.op} ${bStr}`;
  };
  ...
  return (
    <div className="glass-panel p-8 rounded-2xl border-l-4 border-l-emerald-500 overflow-hidden text-white mb-8 flex flex-col items-center">
      <div className="w-full mb-6">
        <h3 className="text-2xl font-bold text-white">Absolute Mastery</h3>
        <p className="text-emerald-400 text-sm font-mono mt-1">Mixed Positive & Negative Arithmetic</p>
      </div>
      ...
      <div className="bg-slate-900 p-6 rounded-2xl w-full max-w-sm border border-slate-700 shadow-2xl relative">
        {isFinished ? (
          <div className="text-center py-6">
            ...
            <div className="text-emerald-400 text-5xl font-mono font-bold my-2">{scoreData.finalScore}</div>
          ...
          <>
            ...
            <div className="bg-slate-800 p-4 rounded-xl mb-6 text-center border border-slate-600 h-24 flex flex-col justify-center items-center">
              <div className="text-3xl font-bold text-white tracking-wider flex items-center justify-center w-full">
                {renderQuestion()} = <span className="text-purple-400 ml-3 bg-slate-900 px-3 py-1 rounded min-w-[3rem] border border-slate-700">{userAnswer || '?'}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <button onClick={handleMinus} className="bg-slate-600 hover:bg-slate-500 text-white font-bold py-4 rounded-xl text-2xl transition-colors active:bg-slate-400">+/-</button>
              ...
              <button onClick={handleSubmit} className="col-span-3 bg-emerald-500 hover:bg-emerald-400 text-white font-bold py-4 rounded-xl text-xl transition-colors shadow-lg shadow-emerald-500/30 active:bg-emerald-300">SUBMIT</button>
            </div>
          </>
        )}
      </div>
