  ...
  return (
    <div className="glass-panel p-6 md:p-10 rounded-2xl border-l-4 border-l-emerald-500 overflow-hidden text-white flex-grow flex flex-col h-full w-full">
      <div className="w-full mb-6 shrink-0">
        <h3 className="text-2xl md:text-4xl font-bold text-white">Absolute Mastery</h3>
        <p className="text-emerald-400 text-sm md:text-base font-mono mt-1">Mixed Positive & Negative Arithmetic</p>
      </div>
        ...
        <div className="bg-slate-900 p-8 md:p-12 rounded-3xl w-full max-w-3xl border border-slate-700 shadow-2xl relative">
          {isFinished ? (
            ...
            <>
              ...
              <div className="bg-slate-800 p-6 md:p-8 rounded-2xl mb-6 md:mb-10 text-center border border-slate-600 h-32 md:h-48 flex flex-col justify-center items-center">
                <div className="text-4xl md:text-6xl lg:text-7xl font-bold text-white tracking-wider flex items-center justify-center w-full">
                  {renderQuestion()} = <span className="text-purple-400 ml-4 bg-slate-900 px-4 py-2 rounded-xl min-w-[4rem] border border-slate-700 shadow-inner">{userAnswer || '?'}</span>
                </div>
              </div>
